      ...
      if (s != "") 
      {
        ...
        if (event == "telemetry") 
        {
          // j[1] is the data JSON object
            ...
            // Based on previous path information,
            ...
            for(int i = 0; i < sensor_fusion.size(); i++)
            {
                ...
                if(d < (2 + 4 * lane + 2) && d > (2 + 4 * lane - 2))//same lane
                {
                    ...
                    double check_veh_s = sensor_fusion[i][5];

                    check_veh_s += (check_speed * .02*(double)pre_size);
                    if(check_veh_s > car_s && (check_veh_s - car_s) < 30)
                    {
                      //avoid collision
                      //ref_vel = 29.5;
                      too_close = true;

                      //left turn without regarding if there is car there
                      
                      if(left_empty && lane > 0)
                      {
                        lane -= 1;
                      }
                      else if(right_empty && lane < 2)//
                      {
                        lane += 1;//
                      }
            ...
            else
            {
              ...
              double pre_ref_x = previous_path_x[pre_size - 2];
              double pre_ref_y = previous_path_y[pre_size - 2];
            ...
            //use spline to interpolate next point
            //create spline
            tk::spline s;
            ...
            //define actrual (x, y) points will use for the planner
            ...
            //calculate how to break up spline point so that to get desired velocity
            double target_x = 30.0;
            double target_y = s(target_x);
            double target_dist = sqrt(target_x * target_x + target_y * target_y);
            double x_add_on = 0;

            //fill up the rest of path after fill it with previous pts, 
            for(int i = 0; i <= 50 - previous_path_x.size(); i++)
            {
              double N = (target_dist / (.02 * ref_vel / 2.24));
              double x_p = x_add_on + (target_x) / N;
              double y_p = s(x_add_on);
              ...
              //transit back to global geometry
